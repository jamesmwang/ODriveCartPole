...
def main():
	...
	pid_runtime_assurance = RuntimeAssurance(force_lim=FORCE_LIM, x_lim=X_LIM, theta_lim=THETA_LIM)
	...
	ctrl_fsm = FSM("ctrl", ["STANDBY", "CART_LIM_1", "CART_LIM_2", "DETECT_NEUTRAL", "ZERO_PENDULUM", "SET_VERTICAL", "MOTOR_ON", "SANDBOX", "PID", "RESET"])
	...
	while(running):
		...
		if time.time() - ctrl_timestamp > CTRL_PERIOD:
			...
			if ctrl_fsm.state == "STANDBY":
				...
				print("Press ctrl+z to exit PID...")
				...
				# Runtime assurance
				if pid_runtime_assurance.check(ctrl_force, state_vector):
