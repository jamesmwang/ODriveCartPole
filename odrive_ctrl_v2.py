        ...
        # ACTIVE: Closed loop control for SANDBOX, PID, LQR, or RL active_mode
            ...
            if self.active_mode == "SANDBOX":
                if self.ctrl_mode == "POSITION":
                    # target = np.sin(time.time())
                    target = self.pendulum_enc_reading
                    self.odrive.axis0.controller.input_pos = target
...
def main():
    ...
    while last_keystroke == "g":
        ...
        # Check for sensor timer expiration
        if time.time() - sensor_timestamp >= SENSOR_PERIOD:
            fsm.read_sensors()
            sensor_timestamp = time.time()

        # Check for control timer expiration
